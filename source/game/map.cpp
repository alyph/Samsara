...
void Map::expand_to_fit_chunk(const IVec2 coords)
{
	...
	else if (coords.x < chunk_bounds.x || coords.x >= (chunk_bounds.x + chunk_bounds.width))
	{
		...
		auto temp_chunks = alloc_simple_array<MapChunk>(chunks.size(), true);
	...
	{
		chunks.insert_zeroes(0, (chunk_bounds.y - coords.y) * chunk_bounds.width);
